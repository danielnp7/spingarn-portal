...
export default async function PortalHomePage() {
  ...
  const { data: projects } = await supabase
    .from("projects")
    .select("id, name, status, deadline, area:areas(name), owner:profiles!owner_id(name)")
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-gradient-to-br from-violet-600 to-violet-800 rounded-2xl p-6 text-white">
        <p className="text-violet-200 text-sm mb-1">Bienvenido al portal de</p>
        <h1 className="text-2xl font-bold mb-1">{client?.name}</h1>
        <p className="text-violet-200 text-sm">{profile.name} · {client?.industry ?? "Cliente Spingarn"}</p>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        {[
          { label: "Proyectos activos", value: activeCount, color: "text-violet-700" },
          { label: "En ejecución", value: projects?.filter(p => p.status === "en_ejecucion").length ?? 0, color: "text-amber-600" },
          { label: "Proyectos totales", value: projects?.length ?? 0, color: "text-gray-700" },
        ].map(s => (
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          ...
          <Link href="/portal/proyectos" className="text-xs text-violet-600 hover:underline">Ver todos →</Link>
        </div>
        <div className="space-y-3">
          ...
          {(projects ?? []).length === 0 && (
            <div className="bg-white rounded-xl p-8 text-center text-gray-400 border border-dashed border-gray-200">
              ...
              <Link href="/portal/solicitudes" className="text-violet-600 text-sm hover:underline mt-2 inline-block">
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        <Link href="/portal/solicitudes"
          className="bg-violet-50 hover:bg-violet-100 border border-violet-200 rounded-xl p-5 transition-colors group">
          <p className="font-semibold text-violet-800 mb-1 group-hover:text-violet-900">Solicitar nuevo servicio</p>
          <p className="text-sm text-violet-600">Envíanos un requerimiento y tu asesor lo atenderá a la brevedad.</p>
        </Link>
